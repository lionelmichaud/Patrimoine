...
extension SocialAccounts {
    ...
    -> ItemSelectionList {
        ...
        switch combination {
            case .assets:
                return firstLine.assets.namedValueTable.values.map({($0.name, true)}) // sélectionné par défaut
            case .liabilities:
                return firstLine.liabilities.values.map({($0.name, true)}) // sélectionné par défaut
            case .both:
    ...
    func getBalanceSheetStackedBarChartDataSet(
        ...
        switch combination {
            case .assets:
                dataEntries += balanceArray.map { // pour chaque année
                    BarChartDataEntry(x       : $0.year.double(),
                                      yValues : $0.assets.namedValueTable.filtredValues(itemSelection: itemSelectionList))
                }
                let labels = firstLine.assets.namedValueTable.filtredHeaders(itemSelection : itemSelectionList)
            ...
            case .both:
                ...
                dataSet = BarChartDataSet(entries: dataEntries,
                                          label: (labels.count == 1 ? labels.first : nil))
                dataSet.stackLabels = firstLine.assets.namedValueTable.filtredHeaders(itemSelection : itemSelectionList)
    ...
    func getBalanceSheetCategoryStackedBarChartDataSet(categoryName: String) -> BarChartDataSet? {
        ...
        if let found = firstLine.assets.namedValueTable.values.first(where: { $0.name == categoryName } ) {
            ...
            print("  nom : \(category)")
            guard let labelsInCategory = firstLine.assets.perCategory[category]?.headersArray else {
                return BarChartDataSet()
            }
            ...
            dataEntries = balanceArray.map { // pour chaque année
                let y = $0.assets.perCategory[category]?.valuesArray
