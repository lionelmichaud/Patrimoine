...
struct Owner : Codable, Hashable {
    ...
    mutating func updateOwnedValue(with totalValue: Double) {
        ownedValue = totalValue * fraction
    }
}
...
extension Owners {
    ...
    mutating func updateOwnersFraction(updateSharedValues: () -> ()) {
        let missingFraction = 1.0 - self.sumOfOwnedFractions
...
struct Ownership: Codable {
    ...
    var fullOwners     : Owners = [Owner(name: "Lionel",  age: 56),
                                   Owner(name: "Vanessa", age: 49)]
    var bareOwners     : Owners = [Owner(name: "Lou-Ann", fraction: 0.6),
                                   Owner(name: "Arthur",  fraction: 0.4)]
    ...
    mutating func updateSharedValues() {
        if isDismembered {
            ...
            usufructOwners.forEach { usufruitier in
                ...
                let ownedValue = totalValue * usufruitier.fraction
            ...
            for idx in 0..<usufructOwners.count {
                // prorata détenu par l'usufruitier
                let ownedValue = totalValue * usufructOwners[idx].fraction
    ...
    /// Calculer la nouvelle répartition de valeur entre les co-ropriétaires, usufruitiers et nu-propriétaires
    /// après une mise à jour de la valeure totale d'un bien
    /// - Parameter totalValue: Valeure totale du bien
    mutating func updateTotalValue(with totalValue: Double) {
        self.totalValue = totalValue
        
        // calculer la nouvelle répartition correspondante
        updateSharedValues()
    }
}
